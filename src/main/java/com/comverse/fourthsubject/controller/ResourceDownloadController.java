...
public class ResourceDownloadController {
	...
	@ResponseBody
	@GetMapping("/download-attach/{boId}/{attachId}")
	public ResponseEntity<?> boardDownloadAttach(@PathVariable int boId, @PathVariable int attachId) {
		try {
			return boardService.boardDownloadAttachFile(boId, attachId);
		} catch (IOException e) {
			e.printStackTrace();
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
		}
