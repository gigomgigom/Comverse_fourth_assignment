						...
						<div class="card-body p-4">
				            <div class="form-group row">
				                ...
				                <div class="col-sm-10 row">
				                	<select id="category" class="form-control col-sm-12" name="isExpose">
				                		<option value=0 ${searchIndex.isExpose == 0 ? 'selected' : '' }>선택</option>
				            ...
				            <div class="form-group row">
				                ...
				                <div class="col-sm-10 d-flex row">
				                    ...
				                    <input type="date" class="form-control col-sm-5" id="dateFrom" name="startDate" value="${searchIndex.startDateSdf}">
				                    <input type="date" class="form-control col-sm-5" id="dateTo" name="endDate" value="${searchIndex.endDateSdf}">
				                </div>
				            </div>
				            <div class="form-group row">
			...
			<section class="content">
				<div class="container-fluid px-3">
					<div class="d-flex justify-content-between row">
						<div class="col-md-5">
							<a href="/admin/manage/general/faq/create?pageNo=${searchIndex.pageNo}&isExpose=${searchIndex.isExpose}&keywordCtg=${searchIndex.keywordCtg}&keyword=${searchIndex.keyword}&dateCtg=${searchIndex.dateCtg}&startDate=${searchIndex.startDateSdf}&endDate=${searchIndex.endDateSdf}&stts=${searchIndex.stts}&rowsPerPage=${searchIndex.rowsPerPage}" class="btn btn-md btn-outline-primary bg-white">신규 등록</a>
						</div>
				...
				<div class="container-fluid p-3">
					<div class="card card-default">
							...
							<table class="table">
								...
								<tbody>
									...
									<c:forEach var="faq" items="${faqList}">
										<tr>
											<td class="text-center">${faq.faqId}</td>
											<td class="text-center"><a href="/admin/manage/general/faq/detail?detailId=${faq.faqId}&pageNo=${searchIndex.pageNo}&isExpose=${searchIndex.isExpose}&keywordCtg=${searchIndex.keywordCtg}&keyword=${searchIndex.keyword}&dateCtg=${searchIndex.dateCtg}&startDate=${searchIndex.startDateSdf}&endDate=${searchIndex.endDateSdf}&stts=${searchIndex.stts}&rowsPerPage=${searchIndex.rowsPerPage}">${faq.title}</a></td>
											<td class="text-center"><fmt:formatDate value="${faq.regDate}" type="date"/></td>
											<td class="text-center">${!faq.faqWriting && (now > faq.exposeStart || exposeStart == null) && (now < faq.exposeEnd || faq.exposeEnd == null) ? '노출' : '비노출' }</td>
											<td class="text-center"><fmt:formatDate value="${faq.exposeStart}" type="date"/></td>
											<td class="text-center"><fmt:formatDate value="${faq.exposeEnd}" type="date"/></td>
										</tr>
									</c:forEach>									
								</tbody>
							</table>
						...
						<div class="card-footer bg-white d-flex justify-content-center">
							<nav>
							  <ul class="pagination">
							    <li class="page-item ${searchIndex.pager.groupNo == 1 ? 'disabled' : '' }"><a class="page-link" href="/admin/manage/general/faq/list?pageNo=${searchIndex.pager.startPageNo-1}&isExpose=${searchIndex.isExpose}&keywordCtg=${searchIndex.keywordCtg}&keyword=${searchIndex.keyword}&dateCtg=${searchIndex.dateCtg}&startDate=${searchIndex.startDateSdf}&endDate=${searchIndex.endDateSdf}&stts=${searchIndex.stts}&rowsPerPage=${searchIndex.rowsPerPage}">Previous</a></li>
							    <c:forEach var="pageNum" items="${searchIndex.pager.pageArray}">
							    	<li class="page-item ${searchIndex.pager.pageNo == pageNum ? 'active' : ''}"><a class="page-link" href="/admin/manage/general/faq/list?pageNo=${pageNum}&isExpose=${searchIndex.isExpose}&keywordCtg=${searchIndex.keywordCtg}&keyword=${searchIndex.keyword}&dateCtg=${searchIndex.dateCtg}&startDate=${searchIndex.startDateSdf}&endDate=${searchIndex.endDateSdf}&stts=${searchIndex.stts}&rowsPerPage=${searchIndex.rowsPerPage}">${pageNum}</a></li>
							    </c:forEach>
							    <li class="page-item ${searchIndex.pager.groupNo == searchIndex.pager.totalGroupNo ? 'disabled' : ''}"><a class="page-link" href="/admin/manage/general/faq/list?pageNo=${searchIndex.pager.endPageNo+1}&isExpose=${searchIndex.isExpose}&keywordCtg=${searchIndex.keywordCtg}&keyword=${searchIndex.keyword}&dateCtg=${searchIndex.dateCtg}&startDate=${searchIndex.startDateSdf}&endDate=${searchIndex.endDateSdf}&stts=${searchIndex.stts}&rowsPerPage=${searchIndex.rowsPerPage}">Next</a></li>
							  </ul>
							</nav>
						</div>
					</div>
