$(document).ready(function () {
		...
		$('#summernote').summernote( {
			...
			callbacks: {
				...
				onMediaDelete : (target) => {
	                ...
	                deletedImageFiles.push({date, fileName});
				}
	...
	$(function () {
		const boCtg = $('#edit-form [name=boCtg]').val();
		const boId = $('#edit-form [name=boId]').val();
		
		$.ajax({
			url: '/admin/board/manage/' + boCtg + '/get-board-content?boId=' + boId,
			type: 'GET',
			success: function(data) {
				$('#summernote').summernote('code', data); // Summernote 에디터에 데이터 넣기
			},
			error: function(e) {
				console.log(e);
			}
...
//입력한 내용으로 구성된 객체 생성
function submitForm(deletedImageFiles) {
	let data = {};
	...
	if(!validate.result) {
		...
		uploadData();
		deletedImageFiles.forEach((file)=> {
			deleteSummernoteImageFile(file.date, file.fileName);
		})
	}
...
function uploadData() {
	...
	$('#file-list li').each(function() {
		...
		formData.append('selectedSavedAttach', attachId.substr(7));
	});
	...
	$.ajax({
		url: '/admin/board/manage/' + boCtg + '/edit-board-detail',
		type: 'POST',
