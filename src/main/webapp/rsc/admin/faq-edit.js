...
function validateForm() {
	...
	if($('#edit-form [name=title]').val() === '') {
		result = false;
		msgList.push('제목');
	}
	if($('#edit-form [name=content]').val() === '') {
			result = false;
			msgList.push('내용');
		}
	if($('#edit-form [name=exposeStart]').val() !== '' && $('#edit-form [name=exposeEnd]').val() !== '') {
		var exposeStart = new Date($('#edit-form [name=exposeStart]').val());
		var exposeEnd = new Date($('#edit-form [name=exposeEnd]').val());
...
function uploadData() {
	...
	$.ajax({
		url: '/admin/manage/general/faq/edit-faq',
		type: 'POST',
